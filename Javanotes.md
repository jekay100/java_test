...
2、抽象类的各种abstract成员方法是用于被子类重写(`override`)。非abstract成员方法被子类直接调用(直接使用`super`即可)。
...
子类继承了父类的属性与方法，但不是继承所有的属性与方法，如父类中用private修饰的部分就不能被子类继承。

重构：抽取出具有相同属性与方法的类的相同部分，创建一个父类，然后别的类都继承自这个父类。
